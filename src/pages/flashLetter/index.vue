<template>
  <div class="messagedetail">
    ...
    <div class="custom-tabs" style="margin-top: 46px;">
      <div v-if="$store.state.type === 1" class="custom-tab-item" :class="{active:active === 0}" @click="active = 0">短信列表</div>
      <div v-if="$store.state.type === 2" class="custom-tab-item" :class="{active:active === 0}" @click="active = 0">闪信列表</div>
      <div v-if="$store.state.type === 1" class="custom-tab-item" :class="{active:active === 1}" @click="active = 1">短信模板</div>
      <div v-if="$store.state.type === 2" class="custom-tab-item" :class="{active:active === 1}" @click="active = 1">闪信模板</div>
    </div>
    <van-search
      v-show="active === 0"
        ...
        <div class="tpl-panel" v-for="(item,index) in list" :key="index">
          <van-row>
            ...
            <van-col :span="4" style="text-align: right;">
              <van-tag :style="{'backgroundColor':item.status ? '#59b0a2' : 'rgb(120,120,120)'}">{{item.status ? '通过' : '待审核'}}</van-tag>
            </van-col>
          </van-row>
