  ...
  <div class="myworks" v-title="$route.meta.title">
    <div class="aui-content aui-padded-15 g-fixed-top">
      <div class="aui-bar aui-bar-btn aui-bar-btn-round ">
        <router-link :to="{name:'allworks'}" class="aui-bar-btn-item" tag="div">全部作品</router-link>
        <div class="aui-bar-btn-item works-active" >我的作品</div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      getWorksList() {
        let uid = uid = 'mylist=1&';
        let _this = this;
        this.askGetData('api/work/?' + uid + 'pageSize=' + pageSize + '&page=' + _this.page, '', function (result) { // 获取 活动列表
